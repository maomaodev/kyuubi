 ...
 *
 * Implementation note: a plain class extending Spark case class [[OrcScan]] to
 * reuse its methods (whose internals differ across Spark 3.3 / 3.4 / 3.5). Two
 * consequences of inheriting a case class:
 *  1. [[equals]] / [[hashCode]] are explicitly overridden: `equals` delegates to
 *     the parent case-class field comparison while `hashCode` keys on
 *     `getClass`, so that `BatchScanExec` / `BroadcastExchange` reuse never
 *     treats two different subclass instances as equal under DPP.

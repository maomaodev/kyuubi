 ...
 *
 * Kept semantically equivalent to [[ParquetScanBuilder]] - filter, aggregate and
...
class KyuubiParquetScanBuilder(
  ...
  // Cannot delegate to `super.pushDataFilters` because Spark 3.3's
  // `ParquetScanBuilder.pushDataFilters` is a no-op (returns the input
  // unchanged); the `convertibleFilters` collapse only happens in
  // `pushedFilters` there. We bridge through `HiveBridgeHelper` so a single
  // source compiles against Spark 3.3 / 3.4 / 3.5 with consistent semantics.
